...
class VoiceRecorderViewModel: NSObject, ObservableObject, AVAudioPlayerDelegate {
    @Published var isShowRemoveDialog: Bool
    @Published var isShowDialog: Bool
    ...
    init(
        isShowRemoveDialog: Bool = false,
        isShowErrorDialog: Bool = false,
        ...
        self.isShowRemoveDialog = isShowRemoveDialog
        self.isShowDialog = isShowErrorDialog
        self.dialogMessage = errorDialogMessage
...
extension VoiceRecorderViewModel {
    func voiceRecorderCellTapped(recordedFile: URL) {
    ...
    func removeButtonTapped() {
        setIsShowRemoveDialog(isShow: true)
    }
    ...
    func removeSelectedVoiceRecord() {
        guard let fileToRemove = selectedRecordedFile,
            ...
            showAlert(message: "선택된 음성 메모 파일을 찾을 수 없습니다.")
            return
        }
        ...
        do {
            ...
            stopPlaying()
            showAlert(message: "선택된 음성 메모 파일을 성공적으로 삭제했습니다.")
        } catch {
            showAlert(message: "선택된 음성 메모 파일 삭제 중 오류가 발생했습니다.")
        }
    }
    
    private func setIsShowRemoveDialog(isShow: Bool) {
        isShowRemoveDialog = isShow
    }
    
    private func setErrorAlertMessage(message: String) {
        dialogMessage = message
    }
    
    private func setIsShowErrorDialog(isShow: Bool) {
        isShowDialog = isShow
    }
    
    private func showAlert(message: String) {
        setErrorAlertMessage(message: message)
        setIsShowErrorDialog(isShow: true)
    }
}
...
extension VoiceRecorderViewModel {
    ...
    private func startRecording() {
        ...
        do {
            ...
            showAlert(message: "음성 메모 녹음 중 오류가 발생했습니다.")
        }
    }
...
extension VoiceRecorderViewModel {
    func startPlaying(recordingURL: URL) {
        do {
            ...
            showAlert(message: "음성 메모 재생 중 오류가 발생했습니다.")
        }
    }
    ...
    func getFileInfo(for url: URL) -> (Date?, TimeInterval?) {
        ...
        do {
            ...
            showAlert(message: "선택된 음성 메모 파일 정보를 불러올 수 없습니다.")
        }
        ...
        do {
            ...
            showAlert(message: "선택된 음성 메모 파일의 재생 시간을 불러올 수 없습니다.")
        }
